...
from moccasin.msig_cli.validators import is_valid_private_key, validate_json_file
...
def run(
    ...
    if account and account.address:
        ...
        if is_right_account.lower() not in ("yes", "y"):
            raise MsigCliUserAbort("User aborted tx_sign command due to wrong account.")
    else:
