...
def test_generate_sql_from_args_with_where(blank_tempdir):
    ...
    expected_sql = "SELECT contract_address,contract_name,rpc,deployer,tx_hash,broadcast_ts,tx_dict,receipt_dict,source_code,nickname,abi,session_id,deployment_id FROM deployments WHERE contract_name = ? AND json_extract(tx_dict, '$.chainId') = ? ORDER BY broadcast_ts DESC LIMIT ? "
...
def test_generate_sql_from_args_without_where(blank_tempdir):
    ...
    expected_sql = "SELECT contract_address,contract_name,rpc,deployer,tx_hash,broadcast_ts,tx_dict,receipt_dict,source_code,nickname,abi,session_id,deployment_id FROM deployments ORDER BY broadcast_ts DESC "
    expected_parametrs = ()
...
def test_generate_sql_from_args_without_and(blank_tempdir):
    ...
    expected_sql = "SELECT contract_address,contract_name,rpc,deployer,tx_hash,broadcast_ts,tx_dict,receipt_dict,source_code,nickname,abi,session_id,deployment_id FROM deployments WHERE contract_name = ? ORDER BY broadcast_ts DESC LIMIT ? "
